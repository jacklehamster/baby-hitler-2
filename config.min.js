...
if(chest){if(!chest.opened){chest.opened=now;game.playSound(SOUNDS.DOOR)}else if(game.now-chest.opened>2000){game.battle=null}}},index:game=>{const{now,chest,situation}=game;return!this.battle&&situation.chestCleared&&situation.chestCleared[game.frontCell()]?3:!chest.opened?0:Math.min(3,Math.floor((now-chest.opened)/100))},onRefresh:(game,sprite)=>{const{now,chest,situation}=game;if(chest.opened){const frame=Math.floor((now-chest.opened)/100);if(frame>4&&!chest.checked){chest.checked=now;const{item,count,image,message}=chest;if(item){game.pickUp({item,count,image,message:message||"",onPicked:game=>{if(game.battle){if(!game.battle.chest){game.chest=null}
game.battle=null}else{if(!situation.chestCleared){situation.chestCleared={}}
situation.chestCleared[game.frontCell()]=now}}})}else{game.showTip(message||null,game=>{if(game.battle){if(!game.battle.chest){game.chest=null}
game.battle=null}else{if(!situation.chestCleared){situation.chestCleared={}}
...
return!0}},"yupa bottle":game=>{game.gotoScene("drink-yupa");return!0},}