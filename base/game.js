	...
	class Game {
		...
		countAssets(loaded) {
			...
			if (loaded) {
				...
				count += Object.keys(CONFIG_FILES).length;
//				count += document.querySelectorAll("script[src^='config/']").length;
		...
		tooManyErrors() {
			if (!this.showedTooManyErrors) {
				...
				showError("Too many load errors. Please check your internet connection then <a href onClick='location.reload()'>refresh</a>.<br>");
			}
		}
