...
cleanSituations(){for(let sceneName in this.data.situation){const sceneSituation=this.data.situation[sceneName];for(let elem in sceneSituation){if(!Object.keys(sceneSituation[elem]).length){delete sceneSituation[elem]}}
if(!Object.keys(sceneSituation).length){delete this.data.situation[sceneName]}}}
loadScene(scene,restoreMapInfo){this.cleanSituations();this.initScene();const{map,sprites,doors,arrowGrid,events,customCursor,onScene,onSceneRefresh,onSceneShot,onSceneHoldItem,onSceneUseItem,onSceneForward,onSceneBackward,onSceneBattle,onScenePunch,onSceneRotate,}=scene;this.map=toMap(map);if(!restoreMapInfo&&this.map){const mapInfo=this.getMapInfo(this.map,this.door);if(mapInfo){const{pos,rotation}=mapInfo;this.pos=pos;this.rotation=rotation}}
