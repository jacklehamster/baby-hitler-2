...
if(!game.data.yupa||game.data.yupa.inBottle){if(!game.sceneData.showGameOver&&frame>150){game.hideCursor=!1;game.gameOver(" “~You're no\n          Superman!”~")}}else{if(!game.sceneData.yupaTalked&&frame>150){game.sceneData.yupaTalked=game.now;game.playSound(SOUNDS.YUPA);game.showTip("Whadda stupid...",game=>{game.hideCursor=!1;game.gameOver()},null,{talker:"yupa"})}}},sprites:[{custom:(game,sprite,ctx)=>ctx.clearRect(0,0,64,64),},...getCommonMaze("_1"),{src:ASSETS.CEILING_HOLE,col:3,row:4,hidden:game=>!game.facingEvent()||!game.facingEvent().ceilinghole||game.rotation%2!==0||game.moving,index:game=>{const frame=Math.floor((game.now-game.sceneTime)/50);if(frame<10){return 0}
...
return pendingTip&&pendingTip.talker==="ernest"&&pendingTip.progress<1?15+Math.floor(now/100)%2:15},side:LEFT,offsetX:game=>{if(game.sceneData.ernestLeave){return Math.max(-30,-((game.now-game.sceneData.ernestLeave)/300))}
...
return!0},onWin:game=>game.findChest(game.now+2000,{item:"bullet",image:ASSETS.GRAB_GUN,message:"I found one bullet for my gun.",hideFromInventory:!0,}),},},},);game.addScene({name:"final-credit",onScene:game=>{game.playTheme(SOUNDS.SOFT_THEME,{volume:.8});game.hideCursor=!0;game.sceneData.credits=`${game.title||" Where in Space is\n  BABY HITLER?"}
...
const{dx,dy,scale}=game.sceneData.fountainTemplate;return 40-64*scale/2+scale*(10-5)},onRefresh:game=>{const{pos}=game;const{x,y}=pos;game.sceneData.fountainTemplate=game.currentScene.getTemplate(game,Math.round((x)/39)*39,Math.round((y-15)/39)*39+7)},init:(game,sprite)=>{sprite.onRefresh(game)},index:game=>{if(!game.sceneData.fountainTemplate){return-1}
...
return!0},onWin:game=>game.findChest(game.now+2000,{item:"coin",image:ASSETS.GRAB_COIN,}),},},},);game.addScene({name:"maze-3",onScene:game=>{game.save();game.data.slimePresent=game.now},arrowGrid:[[null,null,MENU,null,null],[],[LEFT,null,s(1),null,RIGHT],[LEFT,s(7),s(8),s(7),RIGHT],[LEFT,s(7),s(3),s(7),RIGHT],],map:`
			XXXXXXXXXXXXXXX
			XXX........_XXX
			XXX.XXXXXXX.XXX
			XMX.XX3...._XXX
...
return 0},hidden:game=>!game.situation.showHighScore,},{src:ASSETS.TOP_5,col:3,row:4,index:1,offsetY:({situation},{index})=>{const{highscores}=situation;for(let i=0;i<highscores.length;i++){if(highscores[i].player===index){return i*6}}
return 0},hidden:game=>!game.situation.showHighScore,},{src:ASSETS.TOP_5,col:3,row:4,index:2,offsetY:({situation},{index})=>{const{highscores}=situation;for(let i=0;i<highscores.length;i++){if(highscores[i].player===index){return i*6}}
return 0},hidden:game=>!game.situation.showHighScore,},{src:ASSETS.TOP_5,col:3,row:4,index:3,offsetY:({situation},{index})=>{const{highscores}=situation;for(let i=0;i<highscores.length;i++){if(highscores[i].player===index){return i*6}}
return 0},hidden:game=>!game.situation.showHighScore,},{src:ASSETS.TOP_5,col:3,row:4,index:({now,situation})=>{if(situation.initial===3){return 9}
