...
ctx.shadowBlur=0}
displayInventory(){if(this.bagOpening&&(this.frameIndex===2||this.frameIndex===3)){for(let i in this.inventory){if(i!==this.useItem&&(!this.pickedUp||i!==this.pickedUp.item)){const{item,image,col,row}=this.inventory[i];if(item!=='bullet'){this.displayImage(ctx,{src:image,col,row,index:this.frameIndex-1})}}}}
...
if(this.frameIndex===3){if(this.useItem){this.useItem=null}else{for(let i in this.inventory){const{item,image,col,row}=this.inventory[i];if(this.isMouseHover({src:image,index:this.frameIndex-1,col,row},this.mouse)){if(item!=="bullet"){this.useItem=item;this.useItemTime=this.now}}}}
this.arrow=0}
