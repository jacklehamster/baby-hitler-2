...
this.processingBag=!0;this.actions.push({time:now,command:"openbag",onStart:()=>this.bagOpening=!this.bagOpening?1:-this.bagOpening,onDone:game=>{if(game.bagOpening<0){game.bagOpening=0;if(onClose){onClose(game)}}else if(game.bagOpening>0&&game.useItem){game.useItem=null}
...
showLoadError(){const errors=[];for(let src in this.loadError){if(this.loadErrorResolved[src]){errors.push(`<div style="color: #FFFFDD33">✅ <del>Failed to load: ${src}.</del></div>`)}else{errors.push(`<div>⚠️ Failed to load: <a target=_blank href="${src}">${src}.</a></div>`)}}
...
this.displayTextLine(tempCtx,{msg:"try again",x:16,y:40});this.displayTextLine(tempCtx,{msg:"start over",x:14,y:50});this.displayOutlinedImage(tempCtx.canvas,"black",4,2)}}
...
this.arrow=0}
this.openBag(this.now,game=>{this.onSceneHoldItem(game,this.useItem)})}
...
gameOver(message,leaveTheme){if(!this.data.gameOver){this.waitCursor=!0;this.data.gameOver=this.now;this.sceneData.gameOverMessage=message;this.hideCursor=!1;this.delayAction(game=>{this.waitCursor=!1},1000);this.useItem=null;if(!leaveTheme){this.playTheme(null)}}}
...
return!0}
escapeBattle(callback){if(this.currentScene.onEscapeBattle){this.currentScene.onEscapeBattle(this,callback);return}
...
const{x,y,}=this.pos;if(!this.isBlocked(this.pos,0,-1,!0)){game.move(game.now,"backward",game=>{this.waitCursor=!1;this.playTheme(battleTheme,{volume:.2});if(callback)
...
if(arrow==="BAG"||game.touchActive){const door=game.frontDoor();if(door&&door.lock&&game.frameIndex===0&&game.rotation%2===0){const cell=game.frontCell();if(!game.situation.unlocked||!game.situation.unlocked[cell]){return!1}}
...
return Math.floor(now/200)%4},hidden:({battle,now})=>!battle||battle.foe!=foe||battle.foeDefeated&&now-battle.foeDefeated>=2000,onShot:(game,sprite)=>{const{battle,data}=game;if(!battle.invincible){game.damageFoe(1000,{shot:!0})}},combine:(item,game)=>{game.battle.nextAttack=Math.min(game.now+3000,game.battle.nextAttack);if(item==="gun"){game.showTip("I'm out of ammo.",null,50,{removeLock:!0})}else if(item==="photo"){game.playSound(SOUNDS.HUM);game.showTip("Have you seen this baby?",null,50,{removeLock:!0})}else{game.playSound(SOUNDS.HUM);game.showTip(`Would you like this ${item}?`,null,50,{removeLock:!0})}
return!0},}}
...
function standardBattle(){return[{noPunch:(game,sprite)=>game.evaluate(sprite.canEscape),src:ASSETS.ESCAPE,offsetY:5,hidden:game=>{const{battle,arrow,useItem,bagOpening}=game;return!battle||game.data.gameOver||battle.foeDefeated||useItem||bagOpening},onClick:(game,sprite)=>{if(!game.evaluate(sprite.canEscape)){return}
...
game.battle=null}else{if(!situation.chestCleared){situation.chestCleared={}}
situation.chestCleared[game.frontCell()]=now}})}}}},},{custom:(game,sprite,ctx)=>{const{stats,battle}=game.data;ctx.fillStyle="#333333";ctx.fillRect(4,60,56,3);ctx.fillStyle="#aa0022";ctx.fillRect(5,61,54,1);ctx.fillStyle="#bbcc22";ctx.fillRect(5,61,54*stats.life/stats.maxLife,1);if(battle.dummyBattle){return}
ctx.fillStyle="#333333";ctx.fillRect(4,2,56,3);ctx.fillStyle="#770022";ctx.fillRect(5,3,54,1);ctx.fillStyle="#cc22bb";ctx.fillRect(5,3,54*battle.foeLife/battle.foeMaxLife,1)},hidden:({battle,data})=>!battle||data.stats.life<=0||battle.foeDefeated,},{custom:(game,sprite,ctx)=>{const{battle,now}=game;const l=now-battle.foeDefeated;game.displayTextLine(ctx,{msg:`+${battle.gainedXP}xp`,x:25,y:30-Math.round(l/400),})},hidden:({battle,data,now})=>!battle||!battle.foeDefeated||!battle.gainedXP||now-battle.foeDefeated>2000,},{src:ASSETS.LEVEL_UP,index:({now,battle})=>Math.min(3,Math.floor((now-(battle.foeDefeated+2000))/100)),hidden:({battle,data,now})=>!battle||!battle.foeDefeated||!battle.gainedLevel||now-battle.foeDefeated<=2000||now-battle.foeDefeated>4500,alpha:({now,battle})=>Math.max(0,Math.min(1,(battle.foeDefeated+4500-now)/500)),},]}
const consumable={"cake":game=>{game.gotoScene("alien");return!0},"fruit?":game=>{const{stats}=game.data;if(game.data.stats.state.ate&&!game.battle){game.showTip("I'm not hungry.");return!1}else{game.playSound(SOUNDS.EAT);game.addToLife(100,"#663300");game.data.stats.state.ate=game.now;if(!game.battle){game.showTip("Delicious!")}
return!0}},"water bottle":game=>{const{stats}=game.data;if(game.data.stats.state.drank&&!game.battle){game.showTip(["I'm not thirsty.","Physical exercise usually gets me thirsty."]);return!1}else{game.playSound(SOUNDS.DRINK);game.addToInventory({item:"empty bottle",image:ASSETS.GRAB_BOTTLE});game.addToLife(30,"#0099aa");game.data.stats.state.drank=game.now;if(!game.battle){game.showTip("Refreshing!")}
return!0}},"yupa bottle":game=>{game.gotoScene("drink-yupa");return!0},};const CONFIG_FILES={'9-months.js':{size:6298,md5:'85c4f5d49c347550e1eb95e04c1e8978',},'alien.js':{size:3046,md5:'d4eb77a08d67139971354417a07ce15f',},'amari-baby.js':{size:3642,md5:'999d9943026ff53b6c61fb45b71f9b3f',},'arcade-room.js':{size:1035,md5:'5d448d078ba970810f32a18d38d2185a',},'asteroid-field.js':{size:17057,md5:'c34e5dc4086349dc1b0761b7119509aa',},'birthday.js':{size:607,md5:'984e4fd2c0bb01b827c3164c2314434c',},'bring-cake.js':{size:1773,md5:'57c5e6a38e08765a7913b3d9f98bb521',},'bring-it-on.js':{size:1675,md5:'d9c84bac1fea496ff977d41c82dffc1d',},'cake-face.js':{size:605,md5:'54c860fec364386e2e7fc6206c69391f',},'call-gangsta.js':{size:2988,md5:'c73f20ec2c16ecf6d6b73d76369bf701',},'cards-showdown.js':{size:1558,md5:'ab9366a8593403d453f71180ada22428',},'ceiling-maze.js':{size:10759,md5:'27ef14cacd95e87df2b52e9a5ca538e3',},'cell-maze-2.js':{size:715,md5:'1835442fd869556e7595de32c410d1e5',},'cell-maze.js':{size:18065,md5:'db449eccedb8d096750739bdb3fee80b',},'concert.js':{size:7983,md5:'10defcf92c9a2651d9b3313971cb9c2b',},'congrats.js':{size:919,md5:'2ec7897dcb823305a6cbac0a456d6c77',},'crowd.js':{size:2920,md5:'032f27661431d20a85dd3b1fec6ae396',},'deadly-landing.js':{size:1882,md5:'f66fe33793d8de782579ccbc75e77317',},'disk-screen.js':{size:13125,md5:'05559b58fecea3663dc8cba712a979cc',},'doctar-room.js':{size:8934,md5:'c42f26e8d06527c3c1819ad17b833479',},'doctar.js':{size:20311,md5:'80f655cec3bc97b501dbab017d86e8cd',},'doom.js':{size:1795,md5:'2166cd7313aacca4746dfdc22e81c95d',},'drink-yupa.js':{size:3066,md5:'444588c0437f60a0304332d22a73ea45',},'ernest-card-dealing.js':{size:11529,md5:'1ef17a9ac6ceb373885208750151e338',},'ernest.js':{size:3230,md5:'c212aa738f158aaaec4a1ed0d3a7a972',},'evening-date.js':{size:1127,md5:'9bbde1852c630e86bf9cbe0a79e9fc38',},'explore-tavern.js':{size:3671,md5:'3614e69be44ad335bc4db2b5802f176e',},'final-corridor.js':{size:22166,md5:'98e001cd69d620310027a16121f614a0',},'final-credit.js':{size:3429,md5:'0dc760fdba7a066629e1361f0ec27d31',},'final-doom.js':{size:625,md5:'68a1755032d917a369c87d58f32cf7bf',},'final-exit.js':{size:2109,md5:'fb97967c5ea97d9e3ff371187bb54e6a',},'final-fight.js':{size:861,md5:'e1bf9b48ef5854cc8b167e5d3ca5a17f',},'final-planet-world.js':{size:30590,md5:'b03e8310b438aa151b467e35e3dd1d29',},'final-planet.js':{size:1112,md5:'824dd4c22657f421742fbec25111b671',},'final-title.js':{size:306,md5:'aefbaaced3667a46a24d0e5d766c5a45',},'first-prison-cell.js':{size:1066,md5:'9b9a0cc06fa69cddd43ce14c3dc6db65',},'gangsta-check-cards.js':{size:1940,md5:'337b62f9ccf682732628d28094ba437d',},'gangsta-shootout.js':{size:8919,md5:'e6d2b7816fb12f0cde74611b7e62cd19',},'gangsta-table.js':{size:17532,md5:'16bdaa890e65ae232ee8deb9eeb865c6',},'guards-attack.js':{size:1415,md5:'a2b611905fb2f793ba30f0cb442a3227',},'guards-laughing-2.js':{size:523,md5:'feaae7bb40fd4155a1ef25bf7d67760d',},'guards-laughing.js':{size:686,md5:'7e5341af2e1f873bf14e39014a8be691',},'human-woke.js':{size:7114,md5:'0729fdb8d376c0cc54647a8599938dad',},'i-have-no-hand.js':{size:618,md5:'65d623fa7f97af7fbdc3f809878cd2b9',},'in-progress.js':{size:980,md5:'3aaf28f2ad21be44c34c79390494c039',},'inside-tavern.js':{size:912,md5:'f33044e00b42f309597e6ce009eb1df2',},'jail-cell.js':{size:17980,md5:'28aea68587987a311beb349607bc2bfd',},'lab.js':{size:6572,md5:'cc8e4c23b85a2f731b3a0a2d31e927d4',},'land-planet.js':{size:163,md5:'4b41f05243a1be76140b798a21a08e11',},'landscape.js':{size:7633,md5:'9050d50958a29edf4fc3a8885019c7c4',},'leo-end.js':{size:208,md5:'2348222837eee2fe742d4f2a330e2ab9',},'load-screen.js':{size:4291,md5:'ed7a7932629ea504363e2a89719a5694',},'lock-zoom.js':{size:4529,md5:'6737d042e1b42918aafa64b2a4f95d67',},'locker-room.js':{size:2954,md5:'4d3449ae1bb4f83ab3faa4d0f6d6e8c2',},'look-over-there.js':{size:4955,md5:'342e8c0cde29e5ebfc926b37edf60217',},'look-right-left.js':{size:3610,md5:'0a64ac313314b79973bc0c5612f851f7',},'maze-2.js':{size:2832,md5:'5276025f46cd1c926c674920616cfcab',},'maze-3.js':{size:2678,md5:'c6407888d8afa99f5eb08ca06047d144',},'maze-4.js':{size:2450,md5:'b1212a592b62df95fd504fb2523d57ce',},'maze.js':{size:2460,md5:'a1c4d69ae5d1cf36491e3e49617fb6ca',},'meet-dick.js':{size:648,md5:'5a1d470e313cbd7e6f8bdd6ac09a9868',},'mirror.js':{size:5635,md5:'462386bacd9d771326e71d0fe516f901',},'money-shot.js':{size:728,md5:'730fbd7ee6992be4f7856d8ac692e76b',},'monster-in-space.js':{size:5570,md5:'b56ff3ac3357fff426d8043989ebae05',},'morning-after.js':{size:5640,md5:'51775515e20fceaedb4ba18ec0d3ad20',},'name-screen.js':{size:4526,md5:'744ddc979660e619c1e3255d5ac97741',},'origin.js':{size:3780,md5:'4ef4d246954bcab6ddd674a85f9eaca0',},'outside-carnage.js':{size:5916,md5:'be2af1d1b05536abec74a34e93b0e0e1',},'outside.js':{size:2551,md5:'07bad79d27f925b849f537868bb2c01d',},'panic-exit.js':{size:1447,md5:'de8a1373a2ca87d302a317ff3139c401',},'poor-hitman.js':{size:602,md5:'22752cad772440956cb00acbd4a7ea97',},'robot-dial.js':{size:4119,md5:'e871f78c0f7f081206f3e4b0ddfcf088',},'sarlie-planet-world.js':{size:18346,md5:'ca1cdaa227a4bd1c00e69ca5ff8dda91',},'sarlie-planet.js':{size:6672,md5:'cb995af3f12d8e2fd9071c06bd9c4ccf',},'shop.js':{size:1823,md5:'9319a8b29a008e3419f727bfc1ac58c0',},'shopkeepa.js':{size:49679,md5:'1434e36814a9c8fa939ba67cb33ce651',},'slots.js':{size:5249,md5:'340b57d60b6def11f833940c2e961af9',},'space-adventure.js':{size:5599,md5:'7adfd5ba8703bb1439657ea3139a6fac',},'space-travel.js':{size:1861,md5:'ec57e6e42c17ee2f812025b900e7933a',},'stars.js':{size:9448,md5:'8f1f3ce5e6d43c5760120e2f367594cf',},'start-screen.js':{size:9428,md5:'47b26a8d2883978bb672cd8e5697b277',},'stay-here-baby-hitler.js':{size:370,md5:'fb4de4cedfdd8bf129e437e0a4c4eadb',},'surface.js':{size:1739,md5:'e88a4abb9e94896503febfcecf5ffe44',},'tavern-entrance-zoom.js':{size:7285,md5:'9b5d799fe0399ef5eb0f7acc4997ae4a',},'tavern-entrance.js':{size:3953,md5:'4778ab7adc87384c8d0f61c03fe090d5',},'tavern-phone.js':{size:16384,md5:'361574a103985c9e62fc0af3407b256f',},'tavern-posters.js':{size:692,md5:'00c68291970081321383d30bb3b537b2',},'tavern-stranger-zoom.js':{size:16251,md5:'51fc16abd04105877588912f11cac66b',},'temp-end.js':{size:269,md5:'c7e74bc9e2aaed2d8a8e4b85cc5c79fb',},'the-date.js':{size:22274,md5:'071106309a82ea6cf755d292fa3f9219',},'tile-hole.js':{size:791,md5:'49050c2a848cd2ca4e7772c2dfac8e90',},'toilet-monster.js':{size:827,md5:'19b495e0a34be59b9783081cfbabed15',},'toilet-zoom.js':{size:874,md5:'c32eca182c954ba83f0c8024fe00a12f',},'toilets.js':{size:1642,md5:'cf8d318fea614ec7ac863ef9990c6532',},'training-room.js':{size:3048,md5:'15eefaa0dd1b158e967c0a98d19a3d17',},'vending-machine.js':{size:2184,md5:'00d8a2986650c95176bc26bb3fcec90b',},'we-take-care-of-you.js':{size:2838,md5:'cf2b2423efdc704bad3e0ae4018361d6',},'what-so-funny.js':{size:956,md5:'62e927080943b59aa7e58153a09b14ea',},'yupa-sarlie.js':{size:17965,md5:'db3c051d9c73a444d4d3dab97fcc50c4',},'zoom-arcade.js':{size:20791,md5:'c3680871654d9588552822e5beaa357b',},'zoom-guards.js':{size:3923,md5:'62bbd9833c894ca05d1abedc4eccb04d',},'zoom-hottub-2.js':{size:4987,md5:'3a72d209f88a20fa4cf879575c2246f3',},'zoom-hottub.js':{size:4649,md5:'8e000d7a3fe3983ad438f5bb79af387e',},'zoom-shop-monster.js':{size:15320,md5:'199a89326e9753a6e079dea28f651a0e',},'zoom-vending-machine.js':{size:5026,md5:'4a1ef601fca133d9e3b242c1e33008c5',},};const spritesheet={"assets/access-card.png":"generated/spritesheets/sheet2.png|crop:1344,704,64,64","assets/alexa.png":"generated/spritesheets/sheet1.png|crop:640,0,256,256","assets/alien-eater.png":"generated/spritesheets/sheet0.png|crop:960,1152,320,320","assets/amari-baby.png":"generated/spritesheets/sheet3.png|crop:960,960,128,192","assets/arcade-hands.png":"generated/spritesheets/sheet1.png|crop:1024,640,64,64","assets/arcade-room.png":"generated/spritesheets/sheet1.png|crop:960,640,64,64","assets/arcade.png":"generated/spritesheets/sheet1.png|crop:1088,640,64,64","assets/argue-mouth.png":"generated/spritesheets/sheet3.png|crop:192,192,192,192","assets/arrow-backward.png":"generated/spritesheets/sheet3.png|crop:704,1024,128,128","assets/arrow-cursor.png":"generated/spritesheets/sheet0.png|crop:1472,1088,11,22","assets/arrow-forward.png":"generated/spritesheets/sheet3.png|crop:832,1344,128,128","assets/arrow-left.png":"generated/spritesheets/sheet3.png|crop:1088,960,128,128","assets/arrow-right.png":"generated/spritesheets/sheet3.png|crop:1344,1152,128,128","assets/arrow-side.png":"generated/spritesheets/sheet3.png|crop:1216,1152,128,128","assets/baby-hitler-waiter.png":"generated/spritesheets/sheet1.png|crop:896,256,128,128","assets/bag-out.png":"generated/spritesheets/sheet1.png|crop:1216,1344,128,128","assets/bathroom-background.png":"generated/spritesheets/sheet1.png|crop:512,1408,64,64","assets/beard.png":"generated/spritesheets/sheet2.png|crop:576,0,192,192","assets/birthday.png":"generated/spritesheets/sheet1.png|crop:1152,640,64,64","assets/bottle-shards.png":"generated/spritesheets/sheet2.png|crop:1216,0,192,192","assets/bottle-slot.png":"generated/spritesheets/sheet2.png|crop:1024,0,192,192","assets/bottle.png":"generated/spritesheets/sheet2.png|crop:0,448,192,192","assets/bottom.png":"generated/spritesheets/sheet1.png|crop:1216,640,64,64","assets/boy-smile.png":"generated/spritesheets/sheet3.png|crop:384,800,128,128","assets/bring-cake.png":"generated/spritesheets/sheet2.png|crop:256,1216,192,192","assets/bring-it-on.png":"generated/spritesheets/sheet0.png|crop:832,1216,128,128","assets/brusalien.png":"generated/spritesheets/sheet0.png|crop:0,1216,256,256","assets/cafee.png":"generated/spritesheets/sheet2.png|crop:0,256,192,192","assets/cage-opened.png":"generated/spritesheets/sheet0.png|crop:320,1344,128,128","assets/cage.png":"generated/spritesheets/sheet3.png|crop:576,576,128,192","assets/cake-boom.png":"generated/spritesheets/sheet2.png|crop:256,0,192,256","assets/cake-fork.png":"generated/spritesheets/sheet2.png|crop:0,640,192,192","assets/cake-piece.png":"generated/spritesheets/sheet2.png|crop:0,1024,192,192","assets/cake-trash.png":"generated/spritesheets/sheet2.png|crop:256,1024,192,192","assets/candle.png":"generated/spritesheets/sheet3.png|crop:832,1088,128,128","assets/cards-showdown.png":"generated/spritesheets/sheet3.png|crop:704,576,128,192","assets/ceiling-hole-only.png":"generated/spritesheets/sheet3.png|crop:0,0,192,192","assets/ceiling-hole.png":"generated/spritesheets/sheet3.png|crop:384,384,192,192","assets/ceiling-rope-move.png":"generated/spritesheets/sheet2.png|crop:256,832,192,192","assets/close-door.png":"generated/spritesheets/sheet3.png|crop:1088,384,128,128","assets/close-floor.png":"generated/spritesheets/sheet3.png|crop:1216,384,128,128","assets/close-further-side.png":"generated/spritesheets/sheet3.png|crop:1344,384,128,128","assets/close-side-corner.png":"generated/spritesheets/sheet3.png|crop:512,192,128,128","assets/close-side-solid.png":"generated/spritesheets/sheet3.png|crop:640,192,128,128","assets/close-side.png":"generated/spritesheets/sheet3.png|crop:960,384,128,128","assets/close-wall-back.png":"generated/spritesheets/sheet3.png|crop:960,768,128,128","assets/close-wall-solid.png":"generated/spritesheets/sheet3.png|crop:1216,960,128,128","assets/close-wall.png":"generated/spritesheets/sheet1.png|crop:1024,256,128,128","assets/coin-1.png":"generated/spritesheets/sheet2.png|crop:832,704,64,64","assets/coin-2.png":"generated/spritesheets/sheet2.png|crop:1280,704,64,64","assets/coinstart.png":"generated/spritesheets/sheet3.png|crop:1088,1152,128,128","assets/compass.png":"generated/spritesheets/sheet2.png|crop:1408,512,64,128","assets/crater.png":"generated/spritesheets/sheet0.png|crop:320,512,192,256","assets/crawling-boy.png":"generated/spritesheets/sheet2.png|crop:0,832,192,192","assets/dancer.png":"generated/spritesheets/sheet4.png|crop:384,384,96,96","assets/dark-cloth-stranger.png":"generated/spritesheets/sheet0.png|crop:960,832,320,320","assets/date-night.png":"generated/spritesheets/sheet2.png|crop:0,1216,192,192","assets/date-table.png":"generated/spritesheets/sheet1.png|crop:832,704,256,320","assets/dimming-light.png":"generated/spritesheets/sheet4.png|crop:384,256,128,128","assets/doctar-room.png":"generated/spritesheets/sheet0.png|crop:0,896,320,320","assets/doctar.png":"generated/spritesheets/sheet1.png|crop:1088,704,256,320","assets/doctor-lobby-amari.png":"generated/spritesheets/sheet2.png|crop:768,0,128,192","assets/doctor-lobby-sarlie.png":"generated/spritesheets/sheet2.png|crop:896,0,128,192","assets/doctor-lobby.png":"generated/spritesheets/sheet3.png|crop:192,0,192,192","assets/door-open.png":"generated/spritesheets/sheet3.png|crop:0,1088,128,128","assets/doorway.png":"generated/spritesheets/sheet3.png|crop:576,384,128,192","assets/drink-yupa.png":"generated/spritesheets/sheet0.png|crop:512,448,448,448","assets/dummy.png":"generated/spritesheets/sheet1.png|crop:576,960,256,256","assets/dungeon-lock.png":"generated/spritesheets/sheet3.png|crop:960,1152,128,128","assets/dungeon-move-solid.png":"generated/spritesheets/sheet4.png|crop:128,128,128,128","assets/dungeon-move.png":"generated/spritesheets/sheet4.png|crop:0,0,128,128","assets/eater.png":"generated/spritesheets/sheet3.png|crop:0,1344,128,128","assets/electro-dial.png":"generated/spritesheets/sheet3.png|crop:384,192,128,192","assets/ernest.png":"generated/spritesheets/sheet3.png|crop:832,768,128,192","assets/escape.png":"generated/spritesheets/sheet0.png|crop:256,1344,64,64","assets/evening-date.png":"generated/spritesheets/sheet2.png|crop:448,512,192,192","assets/exit-door.png":"generated/spritesheets/sheet1.png|crop:320,1408,64,64","assets/exit-gate.png":"generated/spritesheets/sheet2.png|crop:256,256,192,192","assets/exit-out-carnage.png":"generated/spritesheets/sheet1.png|crop:832,1280,192,192","assets/exit-out.png":"generated/spritesheets/sheet0.png|crop:320,960,192,192","assets/eyes-open.png":"generated/spritesheets/sheet0.png|crop:320,768,192,192","assets/far-door.png":"generated/spritesheets/sheet4.png|crop:256,128,128,128","assets/far-floor.png":"generated/spritesheets/sheet4.png|crop:128,0,128,128","assets/far-map.png":"generated/spritesheets/sheet3.png|crop:1344,960,128,128","assets/far-side-corner.png":"generated/spritesheets/sheet1.png|crop:768,256,128,128","assets/far-side.png":"generated/spritesheets/sheet3.png|crop:1280,0,128,128","assets/far-wall-solid.png":"generated/spritesheets/sheet3.png|crop:384,1312,128,128","assets/far-wall.png":"generated/spritesheets/sheet3.png|crop:0,960,128,128","assets/fembot.png":"generated/spritesheets/sheet1.png|crop:320,1152,256,256","assets/final-exit.png":"generated/spritesheets/sheet1.png|crop:1344,192,64,64","assets/final-planet.png":"generated/spritesheets/sheet1.png|crop:256,448,64,128","assets/flash-screen.png":"generated/spritesheets/sheet3.png|crop:192,640,128,128","assets/floor-ceiling.png":"generated/spritesheets/sheet1.png|crop:256,576,64,64","assets/floor.png":"generated/spritesheets/sheet3.png|crop:192,1024,128,128","assets/further-side.png":"generated/spritesheets/sheet3.png|crop:192,1152,128,128","assets/further-wall.png":"generated/spritesheets/sheet3.png|crop:0,192,128,128","assets/gangsta-check-cards.png":"generated/spritesheets/sheet3.png|crop:0,576,128,128","assets/gate-open.png":"generated/spritesheets/sheet0.png|crop:512,1280,192,192","assets/gate.png":"generated/spritesheets/sheet0.png|crop:256,1408,64,64","assets/grab-access-card.png":"generated/spritesheets/sheet3.png|crop:0,448,128,128","assets/grab-apple.png":"generated/spritesheets/sheet4.png|crop:256,256,128,128","assets/grab-bottle-2.png":"generated/spritesheets/sheet3.png|crop:192,512,128,128","assets/grab-bottle.png":"generated/spritesheets/sheet3.png|crop:0,320,128,128","assets/grab-cake.png":"generated/spritesheets/sheet3.png|crop:1152,0,128,128","assets/grab-coin.png":"generated/spritesheets/sheet3.png|crop:640,0,128,128","assets/grab-compass.png":"generated/spritesheets/sheet3.png|crop:1024,0,128,128","assets/grab-fork.png":"generated/spritesheets/sheet3.png|crop:896,192,128,128","assets/grab-gun.png":"generated/spritesheets/sheet3.png|crop:1024,192,128,128","assets/grab-key.png":"generated/spritesheets/sheet3.png|crop:1152,192,128,128","assets/grab-lighter.png":"generated/spritesheets/sheet3.png|crop:1280,192,128,128","assets/grab-photo-shot.png":"generated/spritesheets/sheet3.png|crop:384,0,128,128","assets/grab-photo.png":"generated/spritesheets/sheet3.png|crop:512,0,128,128","assets/grab-playing-card.png":"generated/spritesheets/sheet3.png|crop:768,192,128,128","assets/grab-rope.png":"generated/spritesheets/sheet3.png|crop:384,1056,128,128","assets/grab-tickets.png":"generated/spritesheets/sheet3.png|crop:768,0,128,128","assets/grab-warp-drive.png":"generated/spritesheets/sheet3.png|crop:384,1184,128,128","assets/grab-water-bottle.png":"generated/spritesheets/sheet3.png|crop:896,0,128,128","assets/grab-yupa-bottle.png":"generated/spritesheets/sheet0.png|crop:1280,1088,192,192","assets/guard.png":"generated/spritesheets/sheet1.png|crop:832,1024,256,256","assets/guards-attack.png":"generated/spritesheets/sheet3.png|crop:192,1280,128,128","assets/guards-laughing.png":"generated/spritesheets/sheet0.png|crop:832,896,128,192","assets/gun.png":"generated/spritesheets/sheet2.png|crop:1344,960,64,64","assets/hitler.png":"generated/spritesheets/sheet2.png|crop:1280,960,64,64","assets/hitman-argues.png":"generated/spritesheets/sheet0.png|crop:1280,1280,192,192","assets/hitman-beard-walk.png":"generated/spritesheets/sheet1.png|crop:1344,1024,96,192","assets/hitman-cake-face.png":"generated/spritesheets/sheet1.png|crop:1344,704,128,192","assets/hitman-from-above.png":"generated/spritesheets/sheet3.png|crop:192,896,128,128","assets/hitman-laugh.png":"generated/spritesheets/sheet3.png|crop:192,768,128,128","assets/hitman-tavern-closeup.png":"generated/spritesheets/sheet0.png|crop:704,1280,128,192","assets/hitman-walk.png":"generated/spritesheets/sheet1.png|crop:1344,0,96,192","assets/hold-gun.png":"generated/spritesheets/sheet2.png|crop:192,256,64,128","assets/hole-bottom.png":"generated/spritesheets/sheet2.png|crop:1216,960,64,64","assets/hole-in-ceiling-back.png":"generated/spritesheets/sheet2.png|crop:1152,960,64,64","assets/hole-in-ceiling-hole.png":"generated/spritesheets/sheet2.png|crop:1088,960,64,64","assets/hole-in-ceiling-rope.png":"generated/spritesheets/sheet2.png|crop:896,1408,64,64","assets/hole.png":"generated/spritesheets/sheet3.png|crop:192,384,128,128","assets/hook-punch.png":"generated/spritesheets/sheet1.png|crop:320,896,256,256","assets/hottub-close-up.png":"generated/spritesheets/sheet0.png|crop:512,0,384,448","assets/hottub-water-wave.png":"generated/spritesheets/sheet2.png|crop:192,384,64,128","assets/hottub.png":"generated/spritesheets/sheet2.png|crop:192,512,64,128","assets/human-hottub.png":"generated/spritesheets/sheet2.png|crop:1408,0,64,128","assets/human-no-hand.png":"generated/spritesheets/sheet3.png|crop:0,832,128,128","assets/human-wah.png":"generated/spritesheets/sheet0.png|crop:320,1152,192,192","assets/inside-tavern-phone.png":"generated/spritesheets/sheet0.png|crop:896,0,320,384","assets/inside-tavern.png":"generated/spritesheets/sheet0.png|crop:1216,0,256,384","assets/jail-360.png":"generated/spritesheets/sheet2.png|crop:640,768,192,192","assets/jail.png":"generated/spritesheets/sheet3.png|crop:0,1216,128,128","assets/key-count.png":"generated/spritesheets/sheet1.png|crop:256,1056,64,64","assets/lab-door.png":"generated/spritesheets/sheet1.png|crop:256,1120,64,64","assets/lab-entrance.png":"generated/spritesheets/sheet2.png|crop:1024,1024,192,192","assets/lab-exit.png":"generated/spritesheets/sheet1.png|crop:256,1184,64,64","assets/lab-monster-battle.png":"generated/spritesheets/sheet2.png|crop:256,640,192,192","assets/lab-monster-through-hole.png":"generated/spritesheets/sheet1.png|crop:832,384,192,256","assets/lab-monster-wake.png":"generated/spritesheets/sheet1.png|crop:320,640,256,256","assets/lab-monster.png":"generated/spritesheets/sheet2.png|crop:832,1408,64,64","assets/lab.png":"generated/spritesheets/sheet0.png|crop:1344,448,128,192","assets/ladder.png":"generated/spritesheets/sheet2.png|crop:1152,1408,64,64","assets/landscape.png":"generated/spritesheets/sheet2.png|crop:1088,1408,64,64","assets/left-guard.png":"generated/spritesheets/sheet2.png|crop:1216,1024,192,192","assets/level-up.png":"generated/spritesheets/sheet3.png|crop:0,704,128,128","assets/light.png":"generated/spritesheets/sheet1.png|crop:256,768,64,64","assets/lighter.png":"generated/spritesheets/sheet2.png|crop:640,256,192,192","assets/lock-back.png":"generated/spritesheets/sheet2.png|crop:1408,1408,64,64","assets/lock-block.png":"generated/spritesheets/sheet2.png|crop:1344,1408,64,64","assets/lock-digit.png":"generated/spritesheets/sheet2.png|crop:256,448,192,192","assets/lock.png":"generated/spritesheets/sheet2.png|crop:1280,1408,64,64","assets/locker-door.png":"generated/spritesheets/sheet0.png|crop:832,1088,128,128","assets/locker-room.png":"generated/spritesheets/sheet2.png|crop:1024,960,64,64","assets/look-right-left.png":"generated/spritesheets/sheet1.png|crop:1088,1024,256,320","assets/machine.png":"generated/spritesheets/sheet1.png|crop:1344,896,128,128","assets/map.png":"generated/spritesheets/sheet1.png|crop:640,256,128,128","assets/maze-rotation-background-solid.png":"generated/spritesheets/sheet2.png|crop:1408,1152,64,64","assets/maze-rotation-background.png":"generated/spritesheets/sheet2.png|crop:1024,1408,64,64","assets/maze-rotation-corner-solid.png":"generated/spritesheets/sheet2.png|crop:1216,1408,64,64","assets/maze-rotation-corner.png":"generated/spritesheets/sheet1.png|crop:256,1312,64,64","assets/maze-rotation-walls-solid.png":"generated/spritesheets/sheet1.png|crop:256,832,64,64","assets/maze-rotation-walls.png":"generated/spritesheets/sheet1.png|crop:256,1248,64,64","assets/menu-disk.png":"generated/spritesheets/sheet1.png|crop:0,1344,128,128","assets/menu-options.png":"generated/spritesheets/sheet3.png|crop:832,960,128,128","assets/menu-out.png":"generated/spritesheets/sheet3.png|crop:704,768,128,128","assets/menu-profile.png":"generated/spritesheets/sheet3.png|crop:1088,1280,128,128","assets/menu-sound-off.png":"generated/spritesheets/sheet3.png|crop:1216,1280,128,128","assets/menu-sound-on.png":"generated/spritesheets/sheet1.png|crop:1280,256,128,128","assets/missing-hand-punch.png":"generated/spritesheets/sheet1.png|crop:1024,384,256,256","assets/money-shot.png":"generated/spritesheets/sheet1.png|crop:128,1344,128,128","assets/monster-planet.png":"generated/spritesheets/sheet2.png|crop:448,1280,192,192","assets/monster.png":"generated/spritesheets/sheet1.png|crop:576,1216,256,256","assets/moon-base-guard.png":"generated/spritesheets/sheet0.png|crop:256,1216,64,128","assets/moon-base.png":"generated/spritesheets/sheet1.png|crop:256,992,64,64","assets/mountains.png":"generated/spritesheets/sheet0.png|crop:448,1344,64,128","assets/multi-arrows.png":"generated/spritesheets/sheet1.png|crop:256,960,64,32","assets/next-scene.png":"generated/spritesheets/sheet1.png|crop:256,896,64,64","assets/no-hand.png":"generated/spritesheets/sheet1.png|crop:1408,192,64,64","assets/operation-room.png":"generated/spritesheets/sheet2.png|crop:448,0,128,192","assets/origin-story.png":"generated/spritesheets/sheet1.png|crop:1152,256,128,128","assets/outdoor.png":"generated/spritesheets/sheet2.png|crop:1216,704,64,64","assets/outside-bg.png":"generated/spritesheets/sheet2.png|crop:1152,704,64,64","assets/outside-carnage.png":"generated/spritesheets/sheet1.png|crop:1280,384,192,256","assets/outside.png":"generated/spritesheets/sheet1.png|crop:896,0,192,256","assets/panic-exit-background.png":"generated/spritesheets/sheet2.png|crop:1344,256,128,192","assets/panic-exit-runner-1.png":"generated/spritesheets/sheet2.png|crop:1216,256,128,192","assets/panic-exit-runner-2.png":"generated/spritesheets/sheet2.png|crop:1088,256,128,192","assets/panic-exit-runner-3.png":"generated/spritesheets/sheet2.png|crop:1216,1216,128,192","assets/panic-exit-runner-4.png":"generated/spritesheets/sheet2.png|crop:960,256,128,192","assets/panic-exit-yupa.png":"generated/spritesheets/sheet2.png|crop:832,256,128,192","assets/pedro.png":"generated/spritesheets/sheet3.png|crop:960,1280,128,128","assets/phone-split.png":"generated/spritesheets/sheet2.png|crop:448,1088,192,192","assets/photo.png":"generated/spritesheets/sheet2.png|crop:448,896,192,192","assets/planet-items.png":"generated/spritesheets/sheet0.png|crop:0,0,512,512","assets/poor-hitman-back.png":"generated/spritesheets/sheet2.png|crop:1408,960,64,64","assets/poor-hitman-guard.png":"generated/spritesheets/sheet2.png|crop:640,1344,128,128","assets/poor-hitman.png":"generated/spritesheets/sheet1.png|crop:1344,1216,128,128","assets/posters.png":"generated/spritesheets/sheet2.png|crop:1088,704,64,64","assets/profile-notification.png":"generated/spritesheets/sheet1.png|crop:1088,1344,128,128","assets/punch-hit.png":"generated/spritesheets/sheet1.png|crop:576,704,256,256","assets/punch.png":"generated/spritesheets/sheet1.png|crop:320,384,256,256","assets/razor.png":"generated/spritesheets/sheet2.png|crop:768,1344,64,128","assets/right-guard.png":"generated/spritesheets/sheet2.png|crop:448,704,192,192","assets/robot-dial.png":"generated/spritesheets/sheet0.png|crop:1280,832,192,256","assets/rope-ceiling.png":"generated/spritesheets/sheet2.png|crop:1024,704,64,64","assets/runner.png":"generated/spritesheets/sheet0.png|crop:1344,704,128,128","assets/sarlie-hospital.png":"generated/spritesheets/sheet0.png|crop:1344,640,128,64","assets/sarlie-planet-bg.png":"generated/spritesheets/sheet2.png|crop:960,704,64,64","assets/sarlie-planet-world-overlay.png":"generated/spritesheets/sheet0.png|crop:832,1344,128,64","assets/sarlie-planet-world.png":"generated/spritesheets/sheet0.png|crop:832,1408,128,64","assets/sarlie-planet.png":"generated/spritesheets/sheet3.png|crop:832,1216,128,128","assets/sarlie-shop.png":"generated/spritesheets/sheet0.png|crop:896,384,128,64","assets/scan-card.png":"generated/spritesheets/sheet2.png|crop:1408,1024,64,128","assets/scanline.png":"generated/spritesheets/sheet0.png|crop:1472,1110,8,8","assets/screen-fracture.png":"generated/spritesheets/sheet2.png|crop:896,704,64,64","assets/shifty-eyes.png":"generated/spritesheets/sheet3.png|crop:1088,768,128,128","assets/shoots.png":"generated/spritesheets/sheet3.png|crop:832,384,128,128","assets/shop-door.png":"generated/spritesheets/sheet1.png|crop:832,640,128,64","assets/shop-monster.png":"generated/spritesheets/sheet2.png|crop:960,1408,64,64","assets/shop.png":"generated/spritesheets/sheet2.png|crop:1280,512,128,192","assets/shopkeepa.png":"generated/spritesheets/sheet0.png|crop:960,448,384,384","assets/shuffle-cards.png":"generated/spritesheets/sheet2.png|crop:1024,1216,192,192","assets/side-buttons.png":"generated/spritesheets/sheet3.png|crop:704,384,128,128","assets/side-far-far.png":"generated/spritesheets/sheet3.png|crop:1344,576,128,128","assets/side-far-map.png":"generated/spritesheets/sheet3.png|crop:1216,576,128,128","assets/side-further-map.png":"generated/spritesheets/sheet3.png|crop:1088,576,128,128","assets/side-map.png":"generated/spritesheets/sheet3.png|crop:960,576,128,128","assets/skeleton-rope.png":"generated/spritesheets/sheet1.png|crop:256,320,64,128","assets/skeleton.png":"generated/spritesheets/sheet1.png|crop:256,1376,64,64","assets/slime.png":"generated/spritesheets/sheet1.png|crop:1088,0,256,256","assets/slots.png":"generated/spritesheets/sheet2.png|crop:0,0,256,256","assets/spaceship-stairs.png":"generated/spritesheets/sheet1.png|crop:0,1088,256,256","assets/spaceship.png":"generated/spritesheets/sheet1.png|crop:1408,256,64,128","assets/speech-out.png":"generated/spritesheets/sheet3.png|crop:1344,768,128,128","assets/stats.png":"generated/spritesheets/sheet3.png|crop:1216,768,128,128","assets/super-punch.png":"generated/spritesheets/sheet1.png|crop:0,832,256,256","assets/switcharoo.png":"generated/spritesheets/sheet3.png|crop:384,928,128,128","assets/tammy-slow.png":"generated/spritesheets/sheet3.png|crop:832,576,128,128","assets/tavern-entrance.png":"generated/spritesheets/sheet1.png|crop:576,384,256,320","assets/tavern-gangsta-card-game.png":"generated/spritesheets/sheet0.png|crop:512,896,320,384","assets/tavern-gangsta-shootout.png":"generated/spritesheets/sheet1.png|crop:320,0,320,384","assets/tavern-gangsta.png":"generated/spritesheets/sheet1.png|crop:0,0,320,320","assets/the-hole.png":"generated/spritesheets/sheet1.png|crop:384,1408,64,64","assets/tile-hole.png":"generated/spritesheets/sheet1.png|crop:448,1408,64,64","assets/tile.png":"generated/spritesheets/sheet2.png|crop:1088,512,192,192","assets/toilet-monster.png":"generated/spritesheets/sheet1.png|crop:0,576,256,256","assets/toilet-zoom-background.png":"generated/spritesheets/sheet0.png|crop:1024,384,64,64","assets/toilet-zoom.png":"generated/spritesheets/sheet1.png|crop:1408,640,64,64","assets/toilets.png":"generated/spritesheets/sheet0.png|crop:1408,384,64,64","assets/top-5.png":"generated/spritesheets/sheet2.png|crop:448,256,192,256","assets/touch-arrows.png":"generated/spritesheets/sheet3.png|crop:384,704,128,96","assets/touch-punch.png":"generated/spritesheets/sheet1.png|crop:1344,640,64,64","assets/treasure-chest.png":"generated/spritesheets/sheet1.png|crop:1344,1344,128,128","assets/vending-machine-apple.png":"generated/spritesheets/sheet2.png|crop:960,512,128,192","assets/vending-machine-bottle.png":"generated/spritesheets/sheet2.png|crop:832,512,128,192","assets/vending-machine-close.png":"generated/spritesheets/sheet1.png|crop:1280,640,64,64","assets/vending-machine-coin-return.png":"generated/spritesheets/sheet1.png|crop:1024,1280,64,128","assets/vending-machine-coin-slot.png":"generated/spritesheets/sheet3.png|crop:1344,1280,128,128","assets/vending-machine-glass.png":"generated/spritesheets/sheet0.png|crop:1280,384,64,64","assets/vending-machine.png":"generated/spritesheets/sheet0.png|crop:1216,384,64,64","assets/waiter.png":"generated/spritesheets/sheet2.png|crop:1344,768,128,192","assets/wood.png":"generated/spritesheets/sheet2.png|crop:640,1152,192,192","assets/writing.png":"generated/spritesheets/sheet2.png|crop:640,960,192,192","assets/yupa-back-walk.png":"generated/spritesheets/sheet3.png|crop:704,896,128,128","assets/yupa-dance.png":"generated/spritesheets/sheet1.png|crop:0,320,256,256","assets/yupa-dry-close.png":"generated/spritesheets/sheet0.png|crop:1088,384,64,64","assets/yupa-dry.png":"generated/spritesheets/sheet3.png|crop:704,1152,128,128","assets/yupa-grab-photo-alone.png":"generated/spritesheets/sheet2.png|crop:1216,768,128,192","assets/yupa-grab-photo.png":"generated/spritesheets/sheet2.png|crop:1344,1216,128,192","assets/yupa-hottub.png":"generated/spritesheets/sheet1.png|crop:256,640,64,128","assets/yupa-in-lab.png":"generated/spritesheets/sheet0.png|crop:0,512,320,384","assets/yupa-in-shop.png":"generated/spritesheets/sheet2.png|crop:832,768,192,256","assets/yupa-shake.png":"generated/spritesheets/sheet2.png|crop:640,512,192,256","assets/yupa-walk.png":"generated/spritesheets/sheet1.png|crop:1440,1024,48,64","assets/yupa-woopsie.png":"generated/spritesheets/sheet3.png|crop:704,1280,128,128","assets/yupa-zoom.png":"generated/spritesheets/sheet2.png|crop:1024,768,192,192","assets/zoom-arcade.png":"generated/spritesheets/sheet0.png|crop:1344,384,64,64","assets/zoom-dick-mad.png":"generated/spritesheets/sheet3.png|crop:576,768,128,128","assets/zoom-dick.png":"generated/spritesheets/sheet3.png|crop:576,896,128,128","assets/zoom-guard-alert.png":"generated/spritesheets/sheet1.png|crop:1024,1408,64,64","assets/zoom-guards.png":"generated/spritesheets/sheet3.png|crop:576,1024,128,128","assets/zoom-shop-monster.png":"generated/spritesheets/sheet3.png|crop:576,1152,128,128","assets/zoom-tavern-door-lady.png":"generated/spritesheets/sheet3.png|crop:576,1280,128,128","assets/zoom-tavern-door-robot.png":"generated/spritesheets/sheet2.png|crop:832,1216,192,192","assets/zoom-tavern-door-smoke.png":"generated/spritesheets/sheet3.png|crop:384,576,128,128","assets/zoom-tavern-door.png":"generated/spritesheets/sheet0.png|crop:1152,384,64,64","assets/zoom-yupa-alone.png":"generated/spritesheets/sheet2.png|crop:832,1024,192,192"};gameConfig.scenes.push({name:"load-screen",startScene:!0,onScene:game=>{game.waitCursor=!0;game.sceneData.startTime=game.now;game.sceneData.estimate=3600000},onSceneRefresh:game=>{const progress=game.currentScene.getProgress(game);if(progress<1&&game.now>game.sceneData.startTime){const progressSpeed=progress/(game.now-game.sceneData.startTime);const estimate=(1-progress)/progressSpeed;if(Math.abs(game.sceneData.estimate-estimate)>10000||estimate<game.sceneData.estimate){}}else if(game.now>game.sceneData.startTime&&game.waitCursor){game.waitCursor=!1;game.countLoadError=0;console.log(`${game.now / 1000} sec.`)}},getProgress:game=>{const numAssetsLoaded=game.countAssets(!0);const totalAssets=game.countAssets();return(numAssetsLoaded)/(totalAssets)},sprites:[{custom:(game,sprite,ctx)=>{ctx.fillStyle="#000000";ctx.fillRect(0,0,64,64)},hidden:game=>game.currentScene.getProgress(game)<1,onClick:game=>{game.playSound(SOUNDS.RANDOM);game.sceneData.gameStarted=game.now;game.delayAction(game=>{game.gotoScene("start-screen");game.save("clear")},1000)}},{custom:(game,sprite,ctx)=>{const progress=game.currentScene.getProgress(game);ctx.clearRect(0,0,64,64);ctx.globalAlpha=.08*progress;for(let i=0;i<10;i++){game.displayTextLine(ctx,{msg:"dobuki studio game",x:1,y:2+i*6,})}
