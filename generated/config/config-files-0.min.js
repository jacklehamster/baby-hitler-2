...
if(chest&&!chest.opened){chest.opened=now;game.playSound(SOUNDS.DRINK)}},index:({now,chest,situation})=>situation.chestCleared?3:!chest.opened?0:Math.min(3,Math.floor((now-chest.opened)/100)),onRefresh:(game,sprite)=>{const{now,chest,situation}=game;if(chest.opened){const frame=Math.floor((now-chest.opened)/100);if(frame>4&&!chest.checked){chest.checked=now;const{item,image,hideFromInventory}=chest;game.pickUp({item,image,message:"",onPicked:game=>{game.battle=null;situation.chestCleared=now},hideFromInventory})}}},},makeYupa(),...standardMenu(),...standardBag(),],doors:{1:{scene:"maze-3",wayDown:!0,exit:(game,{scene})=>game.fadeToScene(scene,{door:2},1000),},2:{lock:!0,scene:"cell-maze",door:2,exit:(game,{scene,door})=>{game.fadeToScene(scene,{door},1000)},},3:{scene:"first-prison-cell",exit:(game,{scene})=>game.fadeToScene(scene,null,1000),},},events:{T:{chest:!0,blocking:!0,onEvent:(game,event)=>{const{data,now}=game;game.findChest(now,{item:"coin",image:ASSETS.GRAB_COIN,cleared:game.situation.chestCleared,})},},},},)