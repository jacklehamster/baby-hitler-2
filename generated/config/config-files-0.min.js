...
const event=game.facingEvent();if(!event||!event.chest){return!0}
return!1},onClick:(game,sprite)=>{const{now,chest,situation}=game;if(situation.chestCleared){return}
if(chest&&!chest.opened){chest.opened=now;game.playSound(SOUNDS.DRINK)}},index:({now,chest,situation})=>situation.chestCleared?3:!chest.opened?0:Math.min(3,Math.floor((now-chest.opened)/100)),onRefresh:(game,sprite)=>{const{now,chest,situation}=game;if(chest.opened){const frame=Math.floor((now-chest.opened)/100);if(frame>4&&!chest.checked){chest.checked=now;const{item,image,hideFromInventory}=chest;game.pickUp({item,image,message:"",onPicked:game=>{game.battle=null;situation.chestCleared=now},hideFromInventory})}}},},...standardMenu(),...standardBag(),],doors:{1:{scene:"jail-cell",exit:(game,{scene})=>{game.fadeToScene(scene,null,game.hasVisited(scene)?1000:3000)},},2:{scene:"maze-2",exit:(game,{scene})=>game.fadeToScene(scene,{door:1},1000),},3:{scene:"training-room",exit:(game,{scene})=>game.fadeToScene(scene,{door:1},1000),},},events:{T:{chest:!0,blocking:!0,onEvent:(game,event)=>{const{data,now}=game;game.findChest(now,{item:"coin",image:ASSETS.GRAB_COIN,cleared:game.situation.chestCleared,})},},},},)