...
game.mouse.x=Math.max(limitLeft,Math.min(limitRight,game.mouse.x));game.mouse.y=Math.max(limitTop,Math.min(limitBottom,game.mouse.y))},custom:(game,sprite,ctx)=>{const cx=50,cy=18;ctx.fillStyle="#888888";ctx.beginPath();ctx.arc(cx,cy,10,0,Math.PI*2);ctx.fill();ctx.fillStyle="#555555";ctx.beginPath();ctx.arc(cx,cy,4,0,Math.PI*2);ctx.fill();ctx.fillStyle="#bbbbbb";ctx.beginPath();ctx.arc(cx,cy,2,0,Math.PI*2);ctx.fill();const{jx,jy}=game.sceneData.joystick;const joyTop=6;ctx.fillStyle="#bbbbbb";ctx.beginPath();ctx.moveTo(cx-2,cy);ctx.lineTo(cx+2,cy);ctx.lineTo(cx+jx+2,cy+jy-joyTop);ctx.lineTo(cx+jx-2,cy+jy-joyTop);ctx.lineTo(cx-2,cy);ctx.closePath();ctx.fill();ctx.strokeStyle="#ffffff";ctx.beginPath();ctx.moveTo(cx-1,cy);ctx.lineTo(cx+jx-1,cy+jy-joyTop);ctx.stroke();ctx.fillStyle="#77bb77";ctx.beginPath();ctx.arc(cx+jx,cy+jy-joyTop,6,0,Math.PI*2);ctx.fill();ctx.fillStyle="#FFFFFF";ctx.beginPath();ctx.arc(cx+jx-2,cy+jy-joyTop-2,2,0,Math.PI*2);ctx.fill()},},JOYBUTTON:{hidden:game=>!game.sceneData.joy,onRefresh:(game,sprite)=>{if(!game.sceneData.joystick){game.sceneData.joystick={}}
...
game.mouseDown=game.sceneData.joystick.pressed},custom:(game,sprite,ctx)=>{const cx=16,cy=18;ctx.fillStyle="#888888";ctx.beginPath();ctx.arc(cx,cy,8,0,Math.PI*2);ctx.fill();ctx.fillStyle="#222222";ctx.beginPath();ctx.arc(cx,cy,7,0,Math.PI*2);ctx.fill();ctx.fillStyle="#bb5555";ctx.beginPath();ctx.arc(cx,cy+1,6,0,Math.PI*2);ctx.fill();const{pressed}=game.sceneData.joystick;const joyTop=pressed?1:4;ctx.fillStyle="#ff7777";ctx.beginPath();ctx.arc(cx,cy-joyTop,6,0,Math.PI*2);ctx.fill()},},DIALOG:{custom:(game,sprite,ctx)=>{},},};function nop(){}
...
if(this.useItem==="gun"&&(!this.hoverSprite||!this.hoverSprite.bag&&!this.hoverSprite.menu)){if(this.countItem('bullet')>0){const{bullet}=this.inventory;this.removeFromInventory('bullet');this.gunFired=this.now;game.playSound(SOUNDS.GUN_SHOT)}else{this.gunFired=0;game.playSound(SOUNDS.DUD)}
...
return}
this.doorOpening=0;this.doorOpened=0;this.moving=now;this.actions.push({time:now,frame:0,command:"move",direction,onStart,onDone:(game,action)=>{this.moving=0;onDone(game,action)},active:!0,started:!1,repeat:0,});const{yupa}=game.data;if(yupa){yupa.rotation=direction==="forward"?(this.rotation+4)%8:this.rotation;if(direction==="forward"){yupa.position=Math.random()<.5?-9:9}}}
...
let hoveredTip=null;if(this.hoverSprite){hoveredTip=this.evaluate(this.hoverSprite.tip,this.hoverSprite);if(hoveredTip){const tip=this.tips[hoveredTip];if(!tip){this.tips[hoveredTip]={text:hoveredTip,time:this.now+1000,speed:80*TEXTSPEEDER,end:0,}}else{tip.end=0}}}
...
if(loaded){count+=this.config.scenes.length}else{count+=document.querySelectorAll("script[src^='config/']").length}
return count}
...
tooManyErrors(){if(!this.showedTooManyErrors){this.showedTooManyErrors=!0;showError("Too many load errors. Please check your internet connection.<br>")}}
...
processText(text,addOnly){if(text.indexOf("~")>=0){text.split("~").forEach(a=>this.processText(a,addOnly));return text}
if(!this.translate){this.translate={}}
if(!this.translate[text]){this.translate[text]={};if(window.updateTranslationLink&&!addOnly){const request=new XMLHttpRequest();request.open("GET",`${window.updateTranslationLink}${encodeURIComponent(text)}`);request.send()}}
return text}
...
delayAction(callback,delay){this.actions.push({time:this.now,command:"delay",onDone:callback,delay,active:!0,started:!1,})}
...
this.waitCursor=!0;this.useItem=null;const battleTheme=this.battle.theme||SOUNDS.CHIN_TOK_THEME;this.playTheme(null);this.battle=null;this.setVisited(!1);this.playSound(SOUNDS.HIT,{volume:.3});for(let i=0;i<5;i++){this.delayAction(game=>{game.playSound(SOUNDS.HIT,{volume:.3})},150*i)}
...
findChest(found,{item,count,image,cleared,message}){this.chest={found,opened:0,checked:0,count,item,image,cleared,message,}}
